    ...
    <div className="bg-transparent">
        ...
        <div className="mx-auto max-w-6xl px-6 lg:px-8">
          <div className="space-y-4">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-600">
              {t.supportPage.channels.eyebrow}
            </p>
            <h2 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
              {t.supportPage.channels.title}
            </h2>
          </div>

          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {t.supportPage.channels.items.map((channel) => (
              <div
                key={channel.title}
      ...
      <section className="bg-gradient-to-b from-blue-50 via-white to-blue-100 py-24 gradient-motion">
        <div className="mx-auto max-w-6xl px-6 lg:px-8">
          <div className="space-y-4">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-600">
              {t.supportPage.services.eyebrow}
            </p>
            <h2 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
              {t.supportPage.services.title}
            </h2>
            <p className="max-w-2xl text-base text-slate-600">
              {t.supportPage.services.description}
            </p>
          </div>

          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {t.supportPage.services.items.map((service) => (
              <div
                key={service.title}
                className="group hover-card rounded-3xl border border-blue-100 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-blue-300 hover:shadow-xl hover:shadow-blue-600/10"
              >
                <h3 className="text-lg font-semibold text-slate-900">
                  {service.title}
                </h3>
      ...
      <section className="py-24">
        <div className="mx-auto max-w-6xl px-6 lg:px-8">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {t.supportPage.faq.items.map((faq) => (
              <div
                key={faq.question}
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-100">
                  {t.supportPage.final.eyebrow}
                </p>
                <h2 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">
                  {t.supportPage.final.title}
                </h2>
                <p className="mt-4 max-w-2xl text-base text-blue-100">
                  {t.supportPage.final.description}
                </p>
              </div>
              <Link
                href="/contact"
